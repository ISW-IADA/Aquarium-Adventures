...
def test_transformer_joins_and_computes():
    ...
    df_info = pl.DataFrame({"tank_id": [1, 2], "capacity_liters": [500, 1000], "fish_species": ["guppy", "discus"]})
    transformer = AquariumTransformer(df_info)
    out_df = transformer.analyze_data(df_main)

    # Check columns exist
    assert all(col in out_df.columns for col in ["avg_pH_per_tank", "num_readings"]), (
